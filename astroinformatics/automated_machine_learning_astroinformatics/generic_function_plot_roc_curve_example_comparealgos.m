function generic_function_plot_roc_curve_example_comparealgos(pred, resp)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Generic MATLAB function to take
...
%
% NOTE: currently does binary classification only
%
% Arguments:
%           pred pedictors
...
%[X,Y,T,~,OPTROCPT,suby,subnames] = perfcurve(resp,diffscore,'true');
[X,Y,T,AUCtree,OPTROCPT,suby,subnames] = perfcurve(resp,diffscore,'true');
...
plot(X,Y)
hold on
plot(OPTROCPT(1),OPTROCPT(2),'ro', 'LineWidth', 3)
...
%% TODO: change this for two classes
