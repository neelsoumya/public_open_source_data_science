...
#
# Usage:
#    python3 multiple_datatypes_deep_learning_keras_uci.py
#
# Adapted from:
...
model.add(Dense(units=i_num_neurons_layer_1, activation=str_activation_layer2, input_dim=4,
...
temp_str_peptide_file = "breast-cancer-wisconsin_MOD_CURATED_categorical_test3.data"
...
i_split_train_test_from = 10
x_train             = temp_peptide_df.iloc[0:i_split_train_test_from, 1:-2]  # temp_peptide_df["epithelial_cell_size"]
x_train_categorical = temp_peptide_df.iloc[0:i_split_train_test_from, -2]    # temp_peptide_df["epithelial_cell_size"]
y_train             = temp_peptide_df.iloc[0:i_split_train_test_from, -1]    # temp_peptide_df["class"]

x_train_array             = np.array(x_train)
x_train_array_categorical = keras.utils.to_categorical(x_train_categorical)
...
# then concatenate numeric and categorical
#x_train_array_concat = []
#x_train_array_concat.append(x_train_array)
#x_train_array_concat.append(x_train_categorical)

# Concatenate features: both numeric and categorical
# NOTE: this is one way to do it, another would be
#   to be use auto-encoders to make sparse representations
#   of categorical features
x_train_array_concat = np.concatenate([x_train_array, x_train_array_categorical], axis=1)
...
history = model.fit(x=x_train_array_concat, y=y_train,
...
#####################################################################################
x_test = temp_peptide_df.iloc[i_split_train_test_from:, 1:-2]  # temp_peptide_df["epithelial_cell_size"]
x_test_categorical = temp_peptide_df.iloc[0:i_split_train_test_from, -2]  # temp_peptide_df["epithelial_cell_size"]
...
x_test_array_categorical = keras.utils.to_categorical(x_test_categorical)

# y_test = to_categorical(y_test)#,2)

# again concatenate both numeric and categorical features
#   NOTE: can again use autoencoders here
x_test_array_concat = np.concatenate([x_test_array, x_test_array_categorical], axis=1)


print("\n ********** Model Evaluation Section ********** \n")
print("Printing model performance details on test set: \n")

loss_and_metrics = model.evaluate(x_test_array_concat, y_test, batch_size=i_batch_size)
...
classes = model.predict(x_test_array_concat, batch_size=i_batch_size)
print(y_test)
print(classes)
model.predict_classes(x_test_array_concat, batch_size=i_batch_size, verbose=0)  # , steps=1)
# pdb.set_trace()
...
probas = model.predict(x_test_array_concat)  # x_test
...
# rf.fit(x_train_array_concat, y_train)
