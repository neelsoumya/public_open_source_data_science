...
#
# Usage:
#    python3 all_categorical_deep_learning_keras_uci.py
#
# Adapted from:
...
model.add(Dense(units=i_num_neurons_layer_1, activation=str_activation_layer2, input_dim=6,
...
temp_str_peptide_file = "breast-cancer-wisconsin_MOD_CURATED_ALLcategorical_test.data"
...
x             = temp_peptide_df.iloc[:, 0]  # temp_peptide_df["epithelial_cell_size"]
x_categorical = temp_peptide_df.iloc[:, -2]    # temp_peptide_df["epithelial_cell_size"]
y             = temp_peptide_df.iloc[:, -1]    # temp_peptide_df["class"]

x_categorical = keras.utils.to_categorical(x_categorical)
x = keras.utils.to_categorical(x)

x_concat = np.concatenate([x, x_categorical], axis = 1)
